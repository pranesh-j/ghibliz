...
import { Upload, Download, Share2, Loader2, LogIn, UserPlus, ArrowRight, RefreshCw, CreditCard, X, ZoomIn } from "lucide-react" // Added ZoomIn if it was used
...
export default function Home() {
  ...
  const handleDownload = async () => {
    ...
    const downloadUrl = imageData.image_url;
    const previewUrlForFallback = imageData.preview_url || processedImage;

    if (!downloadUrl) {
       toast({ title: "Download Error", description: "Full image URL not found.", variant: "error" });
       if (previewUrlForFallback && previewUrlForFallback.startsWith('data:image')) {
          console.log("Attempting to download preview as fallback...");
          const link = document.createElement('a');
          link.href = previewUrlForFallback;
          link.download = `ghiblified-preview-${imageData.id || Date.now()}.jpg`;
          document.body.appendChild(link);
          link.click();
          link.remove();
          toast({ title: "Downloading Preview", description: "Full image unavailable, downloading preview.", variant: "warning" });
          return;
       }
       return;
    }
    try {
        console.log("Attempting direct download via link for URL:", downloadUrl);
        const link = document.createElement('a');
        ...
        if (previewUrlForFallback && previewUrlForFallback.startsWith('data:image')) {
          console.log("Direct download failed, attempting preview download...");
          const link = document.createElement('a');
          link.href = previewUrlForFallback;
          link.download = `ghiblified-preview-${imageData.id || Date.now()}.jpg`;
          document.body.appendChild(link);
          link.click();
          link.remove();
          toast({ title: "Downloading Preview", description: "Full image download failed, downloading preview.", variant: "warning" });
       }
    }
  }
      ...
      <div className="relative z-10 min-h-screen">
        ...
        <section className="pt-3 sm:pt-8 pb-8 sm:pb-16 px-3 sm:px-8">
          <div className="max-w-4xl mx-auto text-center">
            ...
            {/* Upload/Processing Area - Reverted Upload Area Prompt */}
            {!selectedImage ? (
              <div className="p-6 sm:p-8 md:p-14 bg-white/90 backdrop-blur-sm rounded-3xl border border-amber-100 shadow-xl max-w-3xl mx-auto mt-4 sm:mt-8 mb-8 sm:mb-12">
                ...
                />
                {/* Reverted label structure and text */}
                <label
                  htmlFor="image-upload"
                  className={`cursor-pointer block border-2 border-dashed rounded-2xl p-6 sm:p-10 text-center transition-colors ${isProcessing || authLoading ? 'border-gray-200 bg-gray-50 opacity-70 cursor-not-allowed' : 'border-amber-200 hover:border-amber-300'}`}
                >
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-sm sm:text-base text-ghibli-dark/70">Upload JPG or PNG images</p>
                     {/* Removed conditional text and embedded button */}
                  </div>
                </label>
              </div>
            ) : (
