...
function HomeContent() {
  ...
  const handleImageError = useCallback(async (index: number, rowType: 'upper' | 'lower') => {
    ...
    try {
      ...
      // No replacement found — remove the broken item entirely
      setRecentWorks(updatedWorks.filter((_, i) => i !== actualIndex));
    } catch (err) {
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden">
      ...
      <div className="relative z-10 min-h-screen">
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            {!selectedImage ? (
              <>
                ...
                <StylePresets onSelectStyle={handleStyleChange} isAuthenticated={isAuthenticated} />
              </>
            ) : (
        ...
        {(loadingRecentWorks || recentWorks.length > 0) && (
                ...
                <>
                  <div className="relative w-full overflow-hidden mb-3 sm:mb-4">
                    ...
                    <motion.div
                      ...
                      style={{ width: "200%", willChange: "transform", backfaceVisibility: "hidden", transform: "translateZ(0)" }}
                    >
                      {[...recentWorks.slice(0, 6), ...recentWorks.slice(0, 6)].map((item, index) => (
                        <div
                          key={`top-${item.id}-${index}`}
                          className="shrink-0 bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg w-40 sm:w-60 gallery-item border-2 border-transparent"
                        >
